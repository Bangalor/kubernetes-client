...
private[client] case class StatefulSetsApi[F[_]](httpClient: Client[F], server: Uri)(
  implicit
  val F: Sync[F],
  ...
  def namespace(namespace: String) = NamespacedStatefulSetsApi(httpClient, server, namespace)
}
