...
private[client] case class ReplicaSetsApi[F[_]](httpClient: Client[F], server: Uri)(
  implicit
  val F: Sync[F],
  ...
  def namespace(namespace: String) = NamespacedReplicaSetsApi(httpClient, server, namespace)
}
