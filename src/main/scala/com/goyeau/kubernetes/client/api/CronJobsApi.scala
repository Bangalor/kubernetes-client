...
private[client] case class CronJobsApi[F[_]](httpClient: Client[F], server: Uri)(
  implicit
  val F: Sync[F],
  ...
  def namespace(namespace: String) = NamespacedCronJobsApi(httpClient, server, namespace)
}
