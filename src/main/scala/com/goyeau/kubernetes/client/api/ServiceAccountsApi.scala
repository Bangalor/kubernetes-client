...
private[client] case class ServiceAccountsApi[F[_]](httpClient: Client[F], server: Uri)(
  implicit
  val F: Sync[F],
  ...
  def namespace(namespace: String) = NamespacedServiceAccountsApi(httpClient, server, namespace)
}
