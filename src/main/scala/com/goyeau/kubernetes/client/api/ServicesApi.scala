...
private[client] case class ServicesApi[F[_]](httpClient: Client[F], server: Uri)(
  implicit
  val F: Sync[F],
  ...
  def namespace(namespace: String) = NamespacedServicesApi(httpClient, server, namespace)
}
